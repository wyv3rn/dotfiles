...
aliasFromTokens (keyword : tokens) =
  if length tokens < 3 || keyword /= "alias"
    then Nothing
    else
      let (alias : nameAndMail) = tokens
...
filterEmptyLines = filter (not . null)
...
main = do
  ...
  (_, Just hout, _, _) <-
    createProcess
      (shell $ "ssh " ++ host ++ " cat " ++ remotePath)
        { std_out = CreatePipe
        }

  remoteList <- filterEmptyLines . lines <$> hGetContents hout
  ...
  let aliases = mapMaybe aliasFromMuttStr combinedList
